...
- **受控（须 `Authorization: Bearer <token>`）**：`GET /cart`、`POST /cart/add`、`POST /cart/checkout`、`POST /cart/clear`、`GET /my-skills`、`POST /inject`；`GET /audit` 仅 `platform` 角色。
...
## 真实数据注入激活（演示态/激活态，接已交付 9 §七）
- **默认演示态**：技能包模板均为演示数据；`POST /analyze`（带鉴权）返回的 `matchedSkills` 含 `state`（demo/none）与 `usable` 标志，未激活时提示「须由平台协助注入真实数据后方可激活使用（仅持文件/截屏不可用）」。
- **购买 → 演示态**：`POST /cart/checkout` 结算后，所购技能包在该用户下登记为 `demo`（已拥有但未激活）。
- **注入 → 激活态**：`POST /inject { skillId, dataSource? }` 模拟「平台协助注入真实数据」——须先购买，否则 409；成功后该用户技能包翻为 `activated`，`usable=true`，并写**注入审计 + 哈希链存证**（平台 `GET /audit` 可见）。
- **注入适配层（可替换接缝，`src/injection.js`）**：已落为稳定的 `InjectionAdapter` 接口契约——
  `validateDPA(sub,skillId,dataSource)`（DPA/最小必要/目的限定）→ `fetchSource(dataSource)`（ERP/系统 API/文件）→ `etl(raw)`（脱敏/清洗/结构化）→ `encryptBind(clean)`（加密落库+绑定运行时，返回 `{bound,bindingRef}`）。
  默认提供 `DemoInjectionAdapter`（安全占位、不触外网）；生产继承并覆盖方法，调用 `setInjectionAdapter(new ErpInjectionAdapter())` 即可替换，`injectRealData` 签名与返回结构不变。`injectRealData` 返回含 `adapter` 字段标明实际所用适配层。
- **生命周期演示**：`node src/index.js --lifecycle` 一键演示 `none → 购买(demo) → 注入(activated)` 状态切换，并打印 `bindingId` / `auditRef` / 适配层名。
- `GET /my-skills` 查看当前用户的 演示态/激活态 列表。
...
│   ├── audit.js      # 结算审计 + 注入审计 + 哈希链存证（篡改即断裂）
│   ├── activation.js # 激活态存储：按(sub,skillId)记录 演示态/激活态（落盘）
│   ├── injection.js  # 注入适配层（InjectionAdapter 接口 + DemoInjectionAdapter 默认实现）：平台协助注入真实数据→翻为激活态
│   ├── index.js      # CLI 演示入口（分析 + 购物车 + 分成 + 生命周期 --lifecycle）
│   └── server.js     # Web 演示（鉴权/限流/隔离/审计/注入 + 分析→购物车→结算→激活）
├── test/
│   └── api-test.js   # 零依赖综合测试（33 项，已交付 10/11）
└── README.md
```
