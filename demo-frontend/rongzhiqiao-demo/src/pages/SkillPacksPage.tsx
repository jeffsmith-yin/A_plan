...
const SkillPacksPage: React.FC = () => {
  const t = useT();
  return (
    <PageContainer title={t("skill.title", "可复用技能包")}>
      <div className="max-w-4xl mx-auto">
        <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-2xl p-6 mb-8 border border-purple-100">
          <h2 className="font-bold text-lg text-gray-800 mb-2">
            {t("skill.whatTitle", "💡 什么是技能包？")}
          </h2>
          <p className="text-sm text-gray-600">
            {t(
              "skill.whatDesc",
              "每个技能包来自一个真实交付项目——专家经验+AI实现，脱敏封装后可被同类企业快速复用。购买技能包可抵扣完整交付服务费。正式数据由客户自行注入。"
            )}
          </p>
        </div>
          ...
          {skillPacks.map((pack) => (
            ...
            >
              <div className="bg-gradient-to-r from-primary-600 to-primary-700 px-5 py-3">
                <span className="text-xs text-primary-100 font-mono">
                  {pack.version}
                </span>
        ...
        <div className="mt-8 bg-gray-50 rounded-xl p-6 border border-gray-200">
          <h3 className="font-bold text-gray-800 mb-3">
            {t("skill.lifecycleTitle", "🔄 技能包生命周期")}
          </h3>
          <div className="flex items-center gap-3 text-sm text-gray-600">
            <span className="bg-white rounded-lg px-3 py-2 border">
              {t("skill.life1", "项目交付")}
            </span>
            <span>→</span>
            <span className="bg-white rounded-lg px-3 py-2 border">
