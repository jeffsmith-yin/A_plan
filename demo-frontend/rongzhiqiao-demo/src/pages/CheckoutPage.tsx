...
const CheckoutPage: React.FC = () => {
  ...
  if (cart.length === 0 && !paid) {
    return (
      <PageContainer title="结算">
        <div className="text-center py-16">
          <p className="text-gray-500 mb-4">购物车为空，无法结算</p>
          <Button onClick={() => navigate("/market")}>去选购</Button>
        </div>
      </PageContainer>
    );
  }
      ...
      <PageContainer title="支付成功">
        <div className="max-w-md mx-auto text-center py-12">
          ...
          <div className="bg-green-50 rounded-xl p-4 border border-green-200 mb-6">
            <p className="text-sm text-green-700">✅ 支付金额：<strong>¥{total.toLocaleString()}</strong></p>
            <p className="text-xs text-green-500 mt-1">⚠️ DEMO环境，未真实扣款</p>
          </div>
          <div className="flex gap-3 justify-center">
            <Button onClick={() => navigate("/market")} variant="outline">继续选购</Button>
            <Button onClick={() => navigate("/hub")}>🏠 返回首页</Button>
          </div>
        </div>
      </PageContainer>
    );
    ...
    <PageContainer title="结算">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100 mb-6">
          <h3 className="font-bold text-gray-800 mb-3">📋 订单摘要</h3>
        ...
        {/* 支付方式 */}
        <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100 mb-6">
          <h3 className="font-bold text-gray-800 mb-4">💳 选择支付方式</h3>
            ...
            <div className="mt-4 space-y-3 p-4 bg-gray-50 rounded-xl">
              <div>
                ...
                <input type="text" value={cardNumber} onChange={e => setCardNumber(e.target.value)}
                  placeholder="请输入银行卡号" maxLength={19}
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-primary-200" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">持卡人姓名</label>
                <input type="text" value={cardName} onChange={e => setCardName(e.target.value)}
                  placeholder="请输入持卡人姓名"
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-primary-200" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">CVV安全码</label>
                <input type="text" value={cardCvv} onChange={e => setCardCvv(e.target.value)}
                  placeholder="卡背面3位数字" maxLength={4}
        ...
        {method !== "bank" && (
          <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100 mb-6 text-center">
            ...
            <p className="text-xs text-gray-400">请使用{method === "wechat" ? "微信" : "支付宝"}扫描二维码支付</p>
            <p className="text-[10px] text-amber-500 mt-1">⚠️ DEMO环境，点击下方按钮模拟支付成功</p>
          </div>
        )}

        <div className="flex gap-3">
          <Button onClick={() => navigate("/cart")} variant="outline">← 返回购物车</Button>
          <Button onClick={handlePay} size="lg">
            {method === "wechat" ? "💚 确认微信支付" : method === "alipay" ? "💙 确认支付宝支付" : "🏦 确认银行卡支付"}
          </Button>
        </div>

        <p className="text-xs text-gray-400 text-center mt-4">
          ⚠️ DEMO环境，所有支付均为模拟，不会真实扣款
        </p>
      </div>
    </PageContainer>
  );
